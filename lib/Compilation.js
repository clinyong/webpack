...
class Compilation extends Tapable {
	...
	addModule(module, cacheGroup) {
		...
		// reset module, in webpack/lib/Module.js#50L
		module.unbuild();
	...
	processModuleDependencies(module, callback) {
		// dependencies 的每个元素都是一个数组
		// 这个嵌套数组的元素应该是满足都是指向同一个文件
		const dependencies = [];

		function addDependency(dep) {
			for(let i = 0; i < dependencies.length; i++) {
				// isEqualResource 调用的是 ModuleDependency 里面的方法
				// 通过文件的绝对路径名称去判断是否是同一个
